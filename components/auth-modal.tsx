...
export function AuthModal({ isOpen, onClose, defaultTab = "login" }: AuthModalProps) {
  ...
  const { signIn, signUp, signInWithGoogle, isLoading, error, clearError } = useAetherStore()
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <button
              type="button"
              disabled={isLoading}
              onClick={async () => { await signInWithGoogle(`${window.location.origin}/dashboard`) }}
              className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl border border-border/50 bg-white/5 hover:bg-white/10 text-foreground font-semibold text-sm transition-all duration-200 disabled:opacity-50"
            >
              {/* Google G logo */}
            ...
            <div className="flex items-center gap-3 my-2">
              ...
              <span className="text-xs text-muted-foreground/50 font-mono uppercase tracking-wider">or</span>
              <div className="flex-1 h-px bg-border/30" />
            </div>
